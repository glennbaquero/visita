...
class ResourceFetchController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $total_checked_in['online_group'] = $bookings->where('is_walkin', false)->where('is_walkin', false)->whereDate('started_at', $today)->get()->count(); 
        $total_checked_in['walk_in'] = $bookings->where('is_walkin', true)->whereDate('started_at', $today)->get()->count(); 
    	$total_checked_in['walk_in_group'] = $bookings->where('is_walkin', true)->where('is_walkin', false)->whereDate('started_at', $today)->get()->count(); 
