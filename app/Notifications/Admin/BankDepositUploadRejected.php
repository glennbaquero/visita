...
class BankDepositUploadRejected extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                ->subject(config('app.name') . ': ' . 'Visita Reservation Rejected')
                ->greeting('Hello ' . $notifiable->renderName() . ',')
                ->line('Your reservation is rejected.')
                ->line('There is something wrong with the uploaded deposit slip, please review the uploaded file and try again.')
                ->line('Bank Deposit Payment')
    ...
    {
        return [
            'message' => 'Your reservation is rejected something wrong with uploaded file!',
