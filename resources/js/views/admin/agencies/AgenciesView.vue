<template>
	<form-request :submit-url="submitUrl" @load="load" @success="fetch" confirm-dialog sync-on-success>
		...
		<card>
			<template v-slot:header>Agency Information</template>
			...
			<div class="row">
				...
				<div class="form-group col-sm-12 col-md-6">
					...
					<input v-model="item.contact_person" name="contact_person" type="text" class="form-control">
				</div>
			...
			<template v-slot:footer>
                ...
                :message="'Are you sure you want to archive Agency #' + item.id + '?'"
                :alt-message="'Are you sure you want to restore Agency #' + item.id + '?'"
                :disabled="loading"
                @load="load"
                @success="fetch"
                @error="fetch"
